...
# Registered before results. These deliberately reduce capacity and increase
# regularization relative to the rejected 0.65--0.95M-parameter v3 models.
variants=(
...
PYTHONPATH=src "${PYTHON_BIN}" -m unitree_gr00t.ours_option_audit \
  ...
  --output "${ARTIFACT_ROOT}/option_registry.json" \
  --device cuda:0 \
  >"${ARTIFACT_ROOT}/option_registry.log"
