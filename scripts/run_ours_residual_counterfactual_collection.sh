...
if ss -ltn | grep -q ":${PORT} "; then
  echo "Refusing occupied residual-label policy port ${PORT}" >&2
  exit 1
fi
...
server_log="artifacts/Ours/counterfactual/R0-residual-v8-server.log"
...
if ! ss -ltn | grep -q ":${PORT} "; then
  echo "Residual-label B server did not open port ${PORT}" >&2
  exit 1
fi
...
for base_seed in 10007 11007 12007; do
  ...
  if [[ -d "${destination}" ]] && [[ -n "$(find "${destination}" -mindepth 1 -print -quit)" ]]; then
    echo "Refusing incomplete residual corpus: ${destination}" >&2
    exit 1
  fi
  stop_stride=1
